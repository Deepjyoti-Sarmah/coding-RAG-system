...
- Visual direction follows two references the user supplied (Hermes Agent,
  ...
  reindex numbers, the editor checklist) is real CKG content or a
...
- `FeatureGrid.jsx`'s numbered #1–#6 grid is a genuine sequence (the six
  pipeline stages `ckg index` actually runs, in order) — this is also the
  section that answers "how does CKG work."
- `StatsBar.jsx` and the hero's install-box footnote only ever show
  ...
  pricing section, since CKG has neither real user quotes to cite nor
  paid tiers to sell.
...
- Install commands currently point at a `git clone` + `uv tool install .`
  ...
  `pip install code-knowledge-graph` once the package is published to
