...
CKG indexes source trees — including, potentially, files containing
...
## What CKG deliberately does

- **No network egress by default.** Indexing, retrieval, and the MCP
  server run entirely against the local `.ckg/index.sqlite`. The only
  opt-in network calls are to a locally-configured Ollama endpoint
  (`ckg embed`, auto-detected at `http://localhost:11434`) — nothing
...
- **The dashboard requires auth.** `ckg dashboard` checks an HMAC bearer
  token (`CKG_DASHBOARD_TOKEN`) and a `Sec-Fetch-Site` CSRF check on
  every request; it binds to `127.0.0.1` only.
- **The index stays in `.ckg/`.** Nothing is written outside the target
  repository's own directory tree (plus editor config files `ckg init`
  is explicitly asked to write, e.g. `.mcp.json`).

## What CKG does not do
